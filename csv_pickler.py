...
training_tweets = open(os.path.join(sys.path[0], "short_reviews/train.txt"), "r").read()

print(len(training_tweets.split('\n')))
documents = []
for index, tweet in enumerate(training_tweets.split('\n')):

    ### remove id numbers, capture value, skip any word with an @ ( @[^\s]+\s ), capture the rest of the tweet
    pattern =  r"\d+,([a-zA-Z]+),(.*)"
    match_object = re.search(pattern, tweet)
    if not match_object:
        continue
...
training_words = nltk.word_tokenize(training_tweets)
...
pickler(training_set, "featuresets.pickle")
print("featureset length", len(featuresets))
